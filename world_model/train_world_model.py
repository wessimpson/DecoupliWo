...
def parse_args() -> argparse.Namespace:
	...
	p.add_argument("--vae_pretrained", type=str, default="stabilityai/sd-vae-ft-mse")
...
def main() -> None:
	...
	# ── Data ──────────────────────────────────────────────────────
	env_root = Path(args.transitions_root) / args.env
	...
	ds_train = mk_ds(env_root / "train")
	ds_test = mk_ds(env_root / "test")
	...
	while global_step < total_steps:
		for batch in loader:
			...
			global_step += 1
			pbar.update(1)
			pbar.set_postfix(loss=f"{loss.item():.4f}", buf=len(error_buffer))

			# Logging
			if global_step > 0 and global_step % 20 == 0:
				writer.add_scalar("train/loss", loss.item(), global_step)
				writer.add_scalar("train/lr", optimizer.param_groups[0]["lr"], global_step)
				writer.add_scalar("train/gamma", last_gamma_eff, global_step)
				writer.add_scalar("train/error_buffer_size", len(error_buffer), global_step)
			...
			if global_step > 0 and args.validation_every > 0 and global_step % args.validation_every == 0:
				...
				with torch.no_grad():
					...
					decoded = world_model.decode_video(chunk_lat)  # [Bv,N,3,H,W]
					img01 = (decoded[:1, 0].clamp(-1, 1) + 1) * 0.5  # first sample, first frame
					tgt01 = (val_tgt[:1, 0].clamp(-1, 1).to(img01.device) + 1) * 0.5
