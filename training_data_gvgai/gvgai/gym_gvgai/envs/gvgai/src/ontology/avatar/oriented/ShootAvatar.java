...
public class ShootAvatar extends OrientedAvatar
{
    ...
    /**
     * When {@link #fireAllWeapons} fires multiple weapons, spread them by this many degrees
     * around the avatar's current orientation (center shot = facing, flanks = +/- spread/2
     * per slot for 3 weapons). 0 = use each missile type's fixed VGDL orientation.
     */
    ...
    protected boolean usesAngularSpread() {
        return itype != null
                && WeaponSpread.usesAngularSpread(fireAllWeapons, itype.length, spreadDegrees);
    }

    protected Direction weaponSpreadDirection(int idx, int weaponCount) {
        return WeaponSpread.direction(idx, weaponCount, this.orientation, spreadDegrees);
    }

    protected void shoot(Game game, int idx)
    {
        boolean angularSpread = usesAngularSpread();
        Direction shotDir = angularSpread ? weaponSpreadDirection(idx, itype.length) : null;

        Vector2d spawn;
        if (alignShotToOrientation || angularSpread) {
            Vector2d dir = angularSpread ? shotDir.getVector() : this.orientation.getVector();
            dir.normalise();
        ...
        if(newOne != null)
        {
            if (angularSpread && newOne.is_oriented) {
                newOne.orientation = shotDir.copy();
            } else if(alignShotToOrientation && newOne.is_oriented) {
