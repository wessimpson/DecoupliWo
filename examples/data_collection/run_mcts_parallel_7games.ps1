<#
.SYNOPSIS
  Launch 7 separate PowerShell processes, each running MCTS data collection (NumEnvs=1) for one game.

.DESCRIPTION
  Each process runs run_random_data_collection.ps1 with tracks.singlePlayer.advanced.sampleMCTS.Agent
  so --total-timesteps is supported.

  Default output (no -OutputRoot): <repo>/data/transitions/train/<game>/ — one folder per game stem.

.PARAMETER TotalTimesteps
  Frames per game (default: 150000).
...
.EXAMPLE
  .\run_mcts_parallel_7games.ps1

.EXAMPLE
  .\run_mcts_parallel_7games.ps1 -TotalTimesteps 500000 -Scale 0.5
#>
[CmdletBinding()]
param(
